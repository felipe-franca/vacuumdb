...
export PGPASSWORD=enterprise@2020
...
dbs="parkingplus-enterprise-demonstrar"
for db in $dbs; do
    echo "Banco -> $db"
    pPSQL="psql -U postgres -h $HOST_IP -d $db -c "
    declare -a TABLES=$($pPSQL "SELECT DISTINCT tablename  from (select t1.relname as tablename, t2.nspname as schema, t2.nspname || '.' ||t1.relname as schema_table, t1.reltuples as qtd_reg, t1.relowner from pg_catalog.pg_class t1 join pg_catalog.pg_namespace t2 on (t1.relnamespace = t2.oid) where t2.nspname not in ('pg_catalog', 'information_schema', 'pg_toast') and t1.relkind = 'r') as tbs where  pg_total_relation_size(schema_table) >= 606650368" --tuples-only)
